...
function DrawingPage({T,tn,onBack}){
  ...
  const LABEL_SIZE=16;
  const BG_OPTIONS=["#ffffff","#f5f5f5","#fff6fa","#f0f7ff","#eaf8e6"];
  ...
  function requestSizeChange(idx){
    ...
    const hasContent=cells.some(r=>r.some(c=>c!==null));
    if(hasContent)setConfirmSize(idx); else applySize(idx);
  }
  function applySize(idx){
    const{cols:nc,rows:nr}=BOARD_SIZES[idx];
    setSizeIdx(idx);setCells(Array(nr).fill(null).map(()=>Array(nc).fill(null)));
    setHistory([]);setRedoStack([]);setConfirmSize(null);
  }
  function pushHistory(snap){setHistory(h=>[...h.slice(-29),snap]);setRedoStack([]);}
  function undo(){setHistory(h=>{if(!h.length)return h;const p=h[h.length-1];setRedoStack(r=>[...r,cells]);setCells(p);return h.slice(0,-1);});}
  function redo(){setRedoStack(r=>{if(!r.length)return r;const n=r[r.length-1];setHistory(h=>[...h,cells]);setCells(n);return r.slice(0,-1);});}

  function paintCellDirect(prev,row,col){
    if(row<0||col<0||row>=rows||col>=cols)return prev;
    const next=prev.map(r=>[...r]);
    const mc=cols-1-col;
    if(tool==="eraser"){next[row][col]=null;if(mirrorMode&&col!==mc)next[row][mc]=null;}
    else{next[row][col]=pickedColor;if(mirrorMode&&col!==mc)next[row][mc]=pickedColor;}
    return next;
  }
  function getCellFromEvent(e,el){
    const rect=el.getBoundingClientRect();
    const t=e.touches?e.touches[0]:e;
    return{row:Math.floor((t.clientY-rect.top)/cellSize),col:Math.floor((t.clientX-rect.left)/cellSize)};
  }
  function onTouchStart(e){
    if(e.touches.length===2){
      ...
      setPainting(false);lastCellRef.current=null;return;
    }
    if(!pickedColor&&tool!=="eraser")return;
    const{row,col}=getCellFromEvent(e,e.currentTarget);
    lastCellRef.current=null;
    setCells(prev=>{const snap=prev.map(r=>[...r]);pushHistory(snap);const n=paintCellDirect(prev,row,col);lastCellRef.current=row+","+col;return n;});
    setPainting(true);
  }
  function onTouchMove(e){
    if(e.touches.length===2&&pinchRef.current.active){
      ...
      setCellSize(Math.max(6,Math.min(28,Math.round(pinchRef.current.startSize*ratio))));
      return;
    }
    if(!painting)return;
    e.preventDefault();
    ...
    setCells(prev=>paintCellDirect(prev,row,col));
  }
  function onTouchEnd(){pinchRef.current.active=false;setPainting(false);lastCellRef.current=null;}
  function onMouseDown(e){
    if(!pickedColor&&tool!=="eraser")return;
    const{row,col}=getCellFromEvent(e,e.currentTarget);
    setCells(prev=>{const snap=prev.map(r=>[...r]);pushHistory(snap);return paintCellDirect(prev,row,col);});
    lastCellRef.current=row+","+col;setPainting(true);
  }
  ...
  function onMouseUp(){setPainting(false);lastCellRef.current=null;}
  ...
  function cellBorderStyle(ci,ri){
    const rw=(ci+1)%10===0?1.2:(ci+1)%5===0?0.7:0.4;
    const rc=(ci+1)%10===0?"rgba(0,0,0,0.22)":(ci+1)%5===0?"rgba(0,0,0,0.13)":"rgba(0,0,0,0.06)";
    const bw=(ri+1)%10===0?1.2:(ri+1)%5===0?0.7:0.4;
    const bc=(ri+1)%10===0?"rgba(0,0,0,0.22)":(ri+1)%5===0?"rgba(0,0,0,0.13)":"rgba(0,0,0,0.06)";
    return{borderRight:rw+"px solid "+rc,borderBottom:bw+"px solid "+bc};
  }

  function exportCanvas(){
    const cs=Math.max(cellSize,16);
    const canvas=document.createElement("canvas");
    canvas.width=LABEL_SIZE+cols*cs;canvas.height=LABEL_SIZE+rows*cs;
    const ctx=canvas.getContext("2d");
    ...
    cells.forEach((row,ri)=>row.forEach((cid,ci)=>{
      ...
      if(cid){
        ...
        ctx.fillStyle=isDark(color&&color.hex||"#fff")?"rgba(255,255,255,0.85)":"rgba(0,0,0,0.65)";
        ctx.font="bold "+Math.max(5,cs*0.36)+"px sans-serif";ctx.textAlign="center";ctx.textBaseline="middle";
        ctx.fillText(cid,ox+cs/2,oy+cs/2);
      }
      ctx.strokeStyle=(ci+1)%10===0?"rgba(0,0,0,0.26)":(ci+1)%5===0?"rgba(0,0,0,0.14)":"rgba(0,0,0,0.06)";
      ctx.lineWidth=(ci+1)%10===0?1.2:(ci+1)%5===0?0.7:0.4;
    ...
    if(entries.length){
      ...
      const c2=document.createElement("canvas");c2.width=canvas.width;c2.height=canvas.height+statH;
      const ctx2=c2.getContext("2d");ctx2.fillStyle="#fff";ctx2.fillRect(0,0,c2.width,c2.height);ctx2.drawImage(canvas,0,0);
      ...
      entries.forEach(([id,cnt])=>{
        ...
        ctx2.fillStyle="#333";ctx2.font="11px sans-serif";ctx2.fillText(id+"x"+cnt,x+17,y);
  ...
  return(
    <div style={{position:"fixed",inset:0,zIndex:3000,display:"flex",flexDirection:"column",background:T.bg,fontFamily:"'Nunito',sans-serif",color:T.text,userSelect:"none"}}>
      ...
      <div style={{background:T.card,borderBottom:"1px solid "+T.border,padding:"6px 12px",display:"flex",alignItems:"center",gap:6,flexShrink:0,overflowX:"auto"}}>
        ...
        <div style={{display:"flex",alignItems:"center",gap:3,marginLeft:4}}>
          <span style={{fontSize:10,color:T.textMid,fontWeight:700,flexShrink:0}}>底色</span>
          {BG_OPTIONS.map(bg=>(
            <div key={bg} onClick={()=>setCanvasBg(bg)}
              style={{width:16,height:16,borderRadius:4,background:bg,border:"2px solid "+(canvasBg===bg?accentC:"rgba(0,0,0,0.15)"),cursor:"pointer",flexShrink:0}}/>
          ))}
        </div>
        <div style={{flex:1}}/>
      ...
      <div style={{flex:1,minHeight:0,display:"flex",overflow:"hidden"}}>
          ...
          <div style={{width:44,flexShrink:0,background:T.card,borderRight:"1px solid "+T.border,display:"flex",flexDirection:"column",overflowY:"auto",zIndex:2}}>
            {seriesList.map(s=>(
              <button key={s} onClick={()=>setActiveSeries(s)}
                ...
                <div style={{width:22,height:22,borderRadius:6,background:seriesColors(s)[0]&&seriesColors(s)[0].hex||"#ccc",border:"1.5px solid "+T.border}}/>
                <span style={{fontSize:9,fontWeight:800,color:T.textMid}}>{s}</span>
              </button>
            ))}
          </div>
        ):(
          <div style={{width:68,flexShrink:0,background:T.card,borderRight:"1px solid "+T.border,display:"flex",flexDirection:"column",zIndex:2,minHeight:0}}>
            <button onClick={()=>setActiveSeries(null)}
              style={{padding:"8px 0",border:"none",borderBottom:"1px solid "+T.border,background:accentC,color:"#fff",fontSize:11,fontWeight:800,cursor:"pointer",flexShrink:0}}>✓ 完成</button>
            <div style={{flex:1,minHeight:0,overflowY:"auto",WebkitOverflowScrolling:"touch",overscrollBehavior:"contain",padding:"4px 4px calc(110px + env(safe-area-inset-bottom))"}}>
              {seriesColors(activeSeries).map(c=>(
                <div key={c.id} onClick={()=>setPickedColor(c.id)}
        ...
        <div ref={wrapRef} style={{flex:1,overflow:"scroll",position:"relative",background:T.bg}}>
          ...
          {pickedColor&&(
            <div style={{position:"sticky",top:8,left:8,display:"inline-flex",alignItems:"center",gap:5,background:T.card,borderRadius:50,padding:"3px 10px 3px 5px",border:"1px solid "+T.border,zIndex:5,marginBottom:-28,marginLeft:8}}>
              <div style={{width:18,height:18,borderRadius:5,background:ALL_COLORS.find(c=>c.id===pickedColor)&&ALL_COLORS.find(c=>c.id===pickedColor).hex||"#ccc",border:"1px solid rgba(0,0,0,0.1)"}}/>
              <span style={{fontSize:11,fontWeight:800,color:T.text}}>{pickedColor}</span>
            </div>
          )}
          <div style={{display:"inline-block",margin:48}}>
            ...
            <div style={{display:"flex"}}>
              ...
              <div
                ...
                style={{display:"grid",gridTemplateColumns:"repeat("+cols+","+cellSize+"px)",width:cols*cellSize,cursor:tool==="eraser"?"cell":"crosshair",touchAction:"none",border:"1.5px solid rgba(0,0,0,0.20)"}}>
                {cells.map((row,ri)=>row.map((cid,ci)=>{
                  const color=cid?ALL_COLORS.find(c=>c.id===cid):null;
                  const isMirrorAxis=mirrorMode&&ci===Math.floor(cols/2)-1;
                  const bs=cellBorderStyle(ci,ri);
                  return(
                    <div key={ri+"-"+ci} style={{width:cellSize,height:cellSize,background:color?color.hex:canvasBg,boxSizing:"border-box",borderRight:isMirrorAxis?"1.5px solid rgba(100,160,255,0.55)":bs.borderRight,borderBottom:bs.borderBottom,display:"flex",alignItems:"center",justifyContent:"center",overflow:"hidden"}}>
                      {color&&cellSize>=12&&(
            ...
            {Object.entries(colorCount).sort((a,b)=>b[1]-a[1]).map(([id,cnt])=>{
              ...
              return(
                <div key={id} style={{display:"flex",alignItems:"center",gap:3,padding:"2px 6px",borderRadius:50,background:T.accentSoft,border:"1px solid "+T.border}}>
                  ...
                  <span style={{fontSize:10,color:T.textMid}}>x{cnt}</span>
                </div>
              );
            })}
          </div>
      ...
      {confirmSize!==null&&(
        <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.4)",zIndex:100,display:"flex",alignItems:"center",justifyContent:"center"}}>
          <div style={{background:T.card,borderRadius:20,padding:"24px 20px",margin:"0 24px",textAlign:"center"}}>
...
function HelperToolPage({T,onBack}){
  ...
  const [gridInput,setGridInput]=useState({w:"",h:""});
  ...
  function enterGridMode(){
    ...
    setGridInput({w:fmt3(base.w),h:fmt3(base.h)});
    setMode("grid");
  }
  ...
  function resizeGrid(dw=0,dh=0){
    setGridCell(prev=>{
      ...
      setGridInput({w:fmt3(next.w),h:fmt3(next.h)});
      return next;
    });
  }
  function setGridNumber(key,value){
    ...
    setGridInput(prev=>({...prev,[key]:raw}));
    if(raw===""||raw===".")return;
    const num=Number(raw);
    if(!Number.isFinite(num))return;
    setGridCell(prev=>{
      const base=makeGridFromCalibration();
      const next={w:prev?.w ?? base.w,h:prev?.h ?? base.h,[key]:clamp(Number(num.toFixed(3)),1,100)};
      return next;
    });
  }
  function commitGridNumber(key){
    if(key!=="w"&&key!=="h")return;
    setGridInput(prev=>{
      const base=makeGridFromCalibration();
      ...
      const raw=prev[key];
      let num=Number(raw);
      if(!Number.isFinite(num))num=current[key] ?? base[key];
      num=clamp(Number(num.toFixed(3)),1,100);
      setGridCell(old=>({
        w:key==="w"?num:(old?.w ?? base.w),
        h:key==="h"?num:(old?.h ?? base.h)
      }));
      return {...prev,[key]:fmt3(num)};
    });
  }
                ...
                <>
                  ...
                  <div style={{display:"grid",gridTemplateColumns:"repeat(2,1fr)",gap:8,marginBottom:12}}>
                    <label style={{fontSize:10,fontWeight:900,color:T.textMid}}>5格宽
                      <input type="number" inputMode="decimal" step="0.001" value={gridInput.w || fmt3(currentGrid.w)} onChange={e=>setGridNumber("w",e.target.value)} onBlur={()=>commitGridNumber("w")} onKeyDown={e=>{if(e.key==="Enter")e.currentTarget.blur();}} style={numberInput}/>
                    </label>
                    <label style={{fontSize:10,fontWeight:900,color:T.textMid}}>5格高
                      <input type="number" inputMode="decimal" step="0.001" value={gridInput.h || fmt3(currentGrid.h)} onChange={e=>setGridNumber("h",e.target.value)} onBlur={()=>commitGridNumber("h")} onKeyDown={e=>{if(e.key==="Enter")e.currentTarget.blur();}} style={numberInput}/>
                    </label>
                  </div>
                  <div style={{fontSize:11,color:T.textMid,lineHeight:1.6,margin:"-2px 0 8px"}}>也可以用按钮调，宽和高支持长按连续增加/减少；数值框可手动输入，精确到小数点后三位。这里不会再画每个小格，只保留 5 虚线 / 10 实线。</div>
                  <div style={{display:"grid",gridTemplateColumns:"repeat(2,1fr)",gap:8,marginBottom:12}}>
                    <button style={smallBtn} {...repeatProps(()=>resizeGrid(-gridSizeStep,0))}>5格宽 -0.001</button>
                    <button style={smallBtn} {...repeatProps(()=>resizeGrid(gridSizeStep,0))}>5格宽 +0.001</button>
                    <button style={smallBtn} {...repeatProps(()=>resizeGrid(0,-gridSizeStep))}>5格高 -0.001</button>
                    <button style={smallBtn} {...repeatProps(()=>resizeGrid(0,gridSizeStep))}>5格高 +0.001</button>
                  </div>
