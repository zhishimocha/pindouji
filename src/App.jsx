...
const THEMES={
  sky:{
    bg:"#f0f7ff",card:"#ffffff",border:"#cce3ff",
    accent:"#4a9eff",accentLight:"#ddeeff",accentSoft:"#eaf4ff",
    warn:"#f5a623",warnBg:"#fff8ec",warnBorder:"#fde5b0",
    danger:"#ff6b6b",dangerBg:"#fff0f0",dangerBorder:"#ffd0d0",
    text:"#2c4a6e",textMid:"#6a90b8",textLight:"#a8c4e0",
    nav:"#ffffff",navBorder:"#dceeff",barBg:"#dceeff",
    bars:["#4a9eff","#72b4ff","#9acaff","#b8d8ff","#d4eaff"],
    titleIcon:"🫧",switchBtn:"夜空 🌙",
    cardShadow:"0 4px 16px rgba(74,158,255,0.10)",
    floatShadow:"0 8px 32px rgba(74,158,255,0.20)",
    headerBg:"linear-gradient(135deg,#e8f4ff 0%,#f5f0ff 100%)",
    navActiveDot:"#4a9eff",
  },
  night:{
    bg:"#0d1b2e",card:"#152236",border:"#1e3352",
    accent:"#ffd166",accentLight:"#2a2010",accentSoft:"#1e1808",
    warn:"#ffd166",warnBg:"#1e1808",warnBorder:"#3a3010",
    danger:"#ff8fa3",dangerBg:"#1e0810",dangerBorder:"#3a1020",
    text:"#d4e8ff",textMid:"#7a9cc0",textLight:"#304860",
    nav:"#0a1520",navBorder:"#1a2d44",barBg:"#1a2d44",
    bars:["#ffd166","#ffbb44","#ffa533","#ff8f22","#ff7a11"],
    titleIcon:"⭐",switchBtn:"晴天 ☀️",
    cardShadow:"0 4px 16px rgba(0,0,0,0.4)",
    floatShadow:"0 8px 32px rgba(255,209,102,0.15)",
    headerBg:"linear-gradient(135deg,#0d1b2e 0%,#162540 100%)",
    navActiveDot:"#ffd166",
  }
};
...
.bob{display:inline-block;animation:bb 2.2s ease-in-out infinite;}
.bob2{display:inline-block;animation:bb 2.2s ease-in-out infinite 0.7s;}
...
export default function App(){
  ...
  function startEdit(id){if(batch)return;setEditId(id);setEditVal(String(stock[id]));}
  function saveEdit(id){const v=parseInt(editVal);if(!isNaN(v)&&v>=0){const d=stock[id]-v;if(d>0)setUsed(u=>({...u,[id]:u[id]+d}));setStock(s=>({...s,[id]:v}));}setEditId(null);}
  ...
  function applyBatch(){
    const amt=parseInt(bAmt);if(isNaN(amt)||amt<=0)return;
      ...
      <div className="tt" style={{minHeight:"100vh",background:T.bg,fontFamily:"'Nunito',sans-serif",color:T.text,paddingBottom:88}}>
        ...
        <div style={{background:T.headerBg,borderBottom:`1.5px solid ${T.border}`,padding:"12px 18px",position:"sticky",top:0,zIndex:100,display:"flex",alignItems:"center",justifyContent:"space-between"}}>
          ...
          <button className="btn" onClick={()=>setTn(t=>t==="sky"?"night":"sky")}
            style={{padding:"7px 16px",borderRadius:50,border:`1.5px solid ${T.border}`,cursor:"pointer",fontFamily:"'Nunito',sans-serif",fontSize:12,fontWeight:700,color:T.accent,background:T.accentLight}}>
            {T.switchBtn}
          </button>
        </div>
        ...
        <div style={{maxWidth:640,margin:"0 auto",padding:"14px 14px 0"}}>
          ...
          {page==="home"&&<div className="fade">
            ...
            {/* 提醒块 */}
            {[["🟡 即将不足",lowC,T.warnBg,T.warnBorder,T.warn],["🔴 不足",critC,T.dangerBg,T.dangerBorder,T.danger]].map(([title,colors,bg,bd,col])=>(
              <div key={title} className="tt" style={{background:bg,border:`1.5px solid ${bd}`,borderRadius:24,padding:"16px",marginBottom:14}}>
                <div style={{fontSize:13,fontWeight:800,marginBottom:10,display:"flex",alignItems:"center",gap:8,color:T.text}}>
                  {title}
                  <span style={{background:tn==="night"?"rgba(255,255,255,0.06)":"rgba(255,255,255,0.85)",borderRadius:50,padding:"2px 12px",fontSize:11,color:T.textMid,fontWeight:600}}>{colors.length} 个</span>
                </div>
                {colors.length===0
                  ?<div style={{textAlign:"center",color:T.textLight,fontSize:13,padding:"10px 0"}}>暂无 ✨</div>
                  :<div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
                    {colors.map(c=>{
                      const dk=isDark(c.hex);
                      return(
                        <div key={c.id} className="cc" onClick={()=>startEdit(c.id)}
                          style={{background:tn==="night"?"rgba(255,255,255,0.04)":"rgba(255,255,255,0.9)",borderRadius:16,overflow:"hidden",cursor:"pointer",border:`1px solid ${T.border}`}}>
                          <div style={{background:c.hex,height:40,display:"flex",alignItems:"center",justifyContent:"center",position:"relative"}}>
                            {tn==="night"&&<div style={{position:"absolute",inset:0,background:"rgba(0,0,0,0.2)"}}/>}
                            <span style={{fontSize:12,fontWeight:800,color:dk?"rgba(255,255,255,0.9)":"rgba(40,30,20,0.65)",position:"relative"}}>{c.id}</span>
                          </div>
                          <div style={{padding:"8px",textAlign:"center"}}>
                            {editId===c.id
                              ?<input autoFocus value={editVal} onChange={e=>setEditVal(e.target.value)} onBlur={()=>saveEdit(c.id)} onKeyDown={e=>{if(e.key==="Enter")saveEdit(c.id);}} onClick={e=>e.stopPropagation()} style={{...inp({width:"100%",textAlign:"center",fontSize:13,padding:"3px",border:`2px solid ${T.accent}`,borderRadius:8})}}/>
                              :<div style={{fontSize:14,fontWeight:800,color:col}}>{stock[c.id]}g</div>
                            }
            ...
            <div className="tt" style={{background:T.card,border:`1.5px solid ${T.border}`,borderRadius:24,padding:"18px",marginBottom:14,boxShadow:T.cardShadow}}>
              ...
              {top5.length===0
                ?<div style={{textAlign:"center",color:T.textLight,fontSize:13,padding:"16px 0"}}>✨ 更新库存后自动统计</div>
                :top5.map(({s,total},i)=>(
                  <div key={s} onClick={()=>goS(s)} style={{marginBottom:12,cursor:"pointer"}}>
                    <div style={{display:"flex",justifyContent:"space-between",fontSize:13,marginBottom:5,fontWeight:700}}>
                      ...
                      <span style={{color:T.textMid,fontWeight:400}}>{total}g · {gToBeads(total)}粒</span>
                    </div>
          ...
          {page==="stock"&&<div className="fade">
            <div style={{display:"flex",gap:8,marginBottom:12,alignItems:"center"}}>
              <input placeholder="🔍 搜索色号 A5、B12..." value={search}
                onChange={e=>{setSearch(e.target.value);setFSeries(null);}}
                style={{...inp({flex:1,padding:"10px 16px",borderRadius:50,fontSize:13})}}/>
              <select value={sort} onChange={e=>setSort(e.target.value)}
                style={{...inp({padding:"10px 8px",borderRadius:50,fontSize:12,cursor:"pointer"})}}>
              ...
              <button className="btn" onClick={()=>{setBatch(!batch);setSel(new Set());}}
                style={{padding:"10px 14px",borderRadius:50,border:"none",cursor:"pointer",fontFamily:"'Nunito',sans-serif",fontSize:12,fontWeight:700,whiteSpace:"nowrap",background:batch?T.accent:T.accentLight,color:batch?"#fff":T.accent}}>
                {batch?"✕ 退出":"批量"}
              </button>
            </div>
            ...
            {batch&&<div style={{background:T.accentSoft,border:`1px solid ${T.border}`,borderRadius:14,padding:"10px 14px",marginBottom:12,fontSize:13,color:T.accent,fontWeight:700}}>
              🫧 点击色卡勾选{sel.size>0&&<span style={{marginLeft:8}}>· 已选 {sel.size} 个</span>}
            </div>}

            <div style={{fontSize:12,color:T.textLight,marginBottom:10,fontWeight:600}}>共 {filtered.length} 个色号</div>
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
              {filtered.map(c=>{
                const st=getStatus(c.id);const dk=isDark(c.hex);const isSel=sel.has(c.id);
                return(
                  <div key={c.id} className="cc tt"
                    onClick={()=>batch?toggleSel(c.id):startEdit(c.id)}
                    style={{background:T.card,borderRadius:20,overflow:"hidden",cursor:"pointer",
                    ...
                    <div style={{background:c.hex,height:50,display:"flex",alignItems:"center",justifyContent:"center",position:"relative"}}>
                      {tn==="night"&&<div style={{position:"absolute",inset:0,background:"rgba(0,0,0,0.2)"}}/>}
                      <span style={{fontSize:13,fontWeight:800,color:dk?"rgba(255,255,255,0.9)":"rgba(40,30,20,0.6)",position:"relative"}}>{c.id}</span>
                      {batch&&<div style={{position:"absolute",right:8,width:20,height:20,borderRadius:"50%",
                        background:isSel?T.accent:"rgba(255,255,255,0.8)",border:`2px solid ${isSel?T.accent:"rgba(200,200,200,0.9)"}`,
                    ...
                    <div style={{padding:"10px 10px 8px",textAlign:"center"}}>
                      {!batch&&editId===c.id
                        ?<input autoFocus value={editVal} onChange={e=>setEditVal(e.target.value)} onBlur={()=>saveEdit(c.id)} onKeyDown={e=>{if(e.key==="Enter")saveEdit(c.id);}} onClick={e=>e.stopPropagation()} style={{...inp({width:"100%",textAlign:"center",fontSize:15,padding:"4px",border:`2px solid ${T.accent}`,borderRadius:10})}}/>
                        :<div style={{fontSize:15,fontWeight:800,color:gToBeads(stock[c.id])<wC?T.danger:gToBeads(stock[c.id])<wL?T.warn:T.text}}>{stock[c.id]}g</div>
                      }
                      <div style={{fontSize:11,color:T.textLight,marginTop:2}}>{gToBeads(stock[c.id])}粒</div>
                      {used[c.id]>0&&<div style={{fontSize:10,color:T.textLight,marginTop:1}}>已用 {used[c.id]}g</div>}
                    </div>
                  </div>
                );
              })}
        ...
        {batch&&sel.size>0&&<div style={{position:"fixed",bottom:84,left:0,right:0,zIndex:300,display:"flex",justifyContent:"center",padding:"0 14px"}}>
          <div className="tt" style={{background:T.card,border:`1.5px solid ${T.border}`,borderRadius:24,padding:"14px 16px",display:"flex",alignItems:"center",gap:8,flexWrap:"wrap",maxWidth:480,width:"100%",boxShadow:T.floatShadow}}>
            ...
            <div style={{display:"flex",border:`1.5px solid ${T.border}`,borderRadius:50,overflow:"hidden"}}>
              {[["-","－扣除"],["+"," ＋补货"]].map(([d,l])=>(
                <button key={d} onClick={()=>setBDir(d)} style={{padding:"6px 14px",border:"none",cursor:"pointer",fontFamily:"'Nunito',sans-serif",fontSize:13,fontWeight:700,background:bDir===d?T.accent:T.card,color:bDir===d?"#fff":T.textMid,transition:"all 0.15s"}}>{l}</button>
              ))}
            </div>
        ...
        <div className="tt" style={{position:"fixed",bottom:0,left:0,right:0,background:T.nav,borderTop:`1.5px solid ${T.navBorder}`,display:"flex",justifyContent:"space-around",padding:"10px 0 20px",zIndex:200}}>
          {[
            {key:"home",label:"首页",iconA:"🏡",iconI:"🏠"},
            {key:"stock",label:"库存",iconA:"🍬",iconI:"🫙"},
          ].map(n=>{
            const active=page===n.key;
            return(
              <button key={n.key} className="btn" onClick={()=>{setPage(n.key);exitBatch();}}
                style={{display:"flex",flexDirection:"column",alignItems:"center",gap:3,background:"none",border:"none",cursor:"pointer",fontFamily:"'Nunito',sans-serif",padding:"4px 40px"}}>
...
function FoxBtn({T,tn}){
  ...
  function handleClick(){
    const w=LOVE_WORDS[Math.floor(Math.random()*LOVE_WORDS.length)];
    setMsg(w);setVis(true);setBounce(true);setHeart(true);
  ...
  return(
    <div style={{position:"relative",display:"inline-flex",alignItems:"center"}}>
      {vis&&<div style={{
        ...
        background:tn==="sky"?"#ffffff":"#1e3352",
        border:`1.5px solid ${T.border}`,borderRadius:18,
        padding:"9px 16px",fontSize:12,fontWeight:700,color:T.text,
        whiteSpace:"nowrap",boxShadow:T.cardShadow,zIndex:999,
