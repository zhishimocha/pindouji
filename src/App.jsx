...
function DrawingPage({T,tn,onBack}){
  ...
  const [canvasBg,setCanvasBg]=useState("#ffffff"); // 画布底色
  const wrapRef=useRef(null);
  const pinchRef=useRef({active:false,startDist:0,startSize:0});
  const lastCellRef=useRef(null); // 记录上次涂的格子，避免重复触发
  const isFluffyUi=tn==="fluffy";
  const accentC=isFluffyUi?"#d07a94":"#4a9eff";
  const LABEL_SIZE=16;

  const BG_OPTIONS=["#ffffff","#f5f5f5","#e8e8e8","#1a1a1a","#2c4a6e","#fff6fa","#f0f7ff"];
  ...
  // 镜像修复：传入当前cells做镜像，不依赖state
  function paintCellDirect(prevCells,row,col){
    if(row<0||col<0||row>=rows||col>=cols)return prevCells;
    const next=prevCells.map(r=>[...r]);
    const mirCol=cols-1-col;
    if(tool==="eraser"){
      next[row][col]=null;
      if(mirrorMode&&col!==mirCol)next[row][mirCol]=null;
    } else {
      next[row][col]=pickedColor;
      if(mirrorMode&&col!==mirCol)next[row][mirCol]=pickedColor;
    }
    return next;
  }
  ...
  function onTouchStart(e){
    if(e.touches.length===2){
      ...
      setPainting(false);lastCellRef.current=null;
      return;
    }
    ...
    setCells(prev=>{
      const snap=prev.map(r=>[...r]);
      pushHistory(snap);
      const next=paintCellDirect(prev,row,col);
  ...
  function onTouchMove(e){
    ...
    const{row,col}=getCellFromEvent(e,e.currentTarget);
    const key=`${row},${col}`;
    if(lastCellRef.current===key)return; // 同格不重复setState
  ...
  function exportCanvas(){
    ...
    cells.forEach((row,ri)=>row.forEach((cid,ci)=>{
      ...
      const isBig=(ci+1)%10===0,isMid=(ci+1)%5===0;
      const isBigR=(ri+1)%10===0,isMidR=(ri+1)%5===0;
      ctx.strokeStyle=isBig?"rgba(0,0,0,0.28)":isMid?"rgba(0,0,0,0.15)":"rgba(0,0,0,0.06)";
      ctx.lineWidth=isBig?1.2:isMid?0.7:0.4;
      ctx.strokeRect(ox,oy,cs,cs);
      if(isBigR){ctx.strokeStyle="rgba(0,0,0,0.28)";ctx.lineWidth=1.2;ctx.beginPath();ctx.moveTo(ox,oy+cs);ctx.lineTo(ox+cs,oy+cs);ctx.stroke();}
      else if(isMidR){ctx.strokeStyle="rgba(0,0,0,0.15)";ctx.lineWidth=0.7;ctx.beginPath();ctx.moveTo(ox,oy+cs);ctx.lineTo(ox+cs,oy+cs);ctx.stroke();}
    }));
                ...
                {cells.map((row,ri)=>row.map((cid,ci)=>{
                  ...
                  return(
                    <div key={`${ri}-${ci}`} style={{
                      ...
                      background:color?color.hex:canvasBg,
                      boxSizing:"border-box",
                      ...cellBorderStyle(ci,ri),
                      ...(isMirrorAxis?{borderRight:"1.5px solid rgba(100,160,255,0.55)"}:{}),
