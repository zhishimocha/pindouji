...
function MissingColorPage({T,stock,onBack}){
  ...
  return(
    <div className="fade" style={{padding:"18px 16px",fontFamily:"'Nunito',sans-serif"}}>
      {cropImg&&(
        <div style={{position:"fixed",inset:0,zIndex:9999,background:"rgba(0,0,0,0.82)",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"flex-start",padding:"12px 12px 18px",overflow:"hidden"}}>
          <div style={{fontSize:13,color:"#fff",fontWeight:800,marginBottom:10,textAlign:"center",flexShrink:0}}>拖拽边框调整选区 · 框选统计表区域</div>
          <div style={{width:"100%",maxWidth:760,flex:1,minHeight:0,display:"flex",alignItems:"center",justifyContent:"center"}}>
            <div
              style={{position:"relative",maxWidth:"100%",maxHeight:"100%",overflow:"hidden",borderRadius:14}}
              onPointerMove={ev=>{
                if(!cropDrag||!cropImgRef.current)return;
                const el=cropImgRef.current.getBoundingClientRect();
                const cx=Math.max(0,Math.min(ev.clientX-el.left,el.width));
            ...
            >
              <img
                ...
                style={{display:"block",maxWidth:"100%",maxHeight:"100%",width:"auto",height:"auto",objectFit:"contain",userSelect:"none"}}
              />
              {cropBox&&(<>
